...
import type { Profile, BarangayRef } from '@/types/profile'
...
export function useAuth() {
  ...
  // ✅ this fixes TS2339 (Property 'user' does not exist...)
  ...
  const signup = async (payload: SignupPayload) => {
    ...
    try {
      const needsBarangay =
        payload.role === 'senior' || payload.role === 'barangay_staff'
      ...
      const profileRow: Omit<Profile, 'barangays'> = {
        id: data.user.id,
        role: payload.role,
barangay_id: needsBarangay ? (payload.barangay_id ?? null) : null,
      ...
      profile.value = { ...profileRow, barangays: null }
